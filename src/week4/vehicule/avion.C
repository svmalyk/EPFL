/*---------------------------------------------------------------------------*\
 #####  #     # |
 #      # # # # | Project Name: Description of the project
 #####  #  #  # | Source Code URL:
     #  #     # | Copyright (C) 2023-2023
 #####  #     # |
-------------------------------------------------------------------------------
License
    This file is part of @ProjectName.
    @ProjectName is free software: you can redistribute it and/or modify it
    under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.
    @ProjectName is distributed in the hope that it will be useful, but WITHOUT
    ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
    FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
    for more details.
    You should have received a copy of the GNU General Public License
    along with @ProjectName. If not, see <http://www.gnu.org/licenses/>.
coding rules:
    Max nb. of lines: 80
    case: Camel Case (other cases: )
    attributes: end with a underscore (eg: attributeName_)
implementation file:
    implementation file contains implementation of all functions declared in
    the header file
Class
    @namespace::example
Description
    description of the class.
implementation file:
    voiture.C
\*---------------------------------------------------------------------------*/
#include "avion.H"

 
// Default Constructor
    

// Overloaded Constructor - 

  Avion::Avion(
        std::string marque_i, 
        int yearOfPurchase_i,  
        double purchaseAmount_i, 
        std::string type_i,
        int heuresDeVol_i
          ): 
        Vehicule(marque_i,
                 yearOfPurchase_i,
                 purchaseAmount_i),
        type_(type_i),
        heuresDeVol_(heuresDeVol_i)
    {}          

// destructeur
    Avion::~Avion() = default;

// getters


 
// setters

// methods

void Avion::calculePrix(){
    double decote;
    if (type_ == "REACTION") decote = static_cast<double>(heuresDeVol_)/1000*0.1;
    if (type_ == "HELICES")  decote = static_cast<double>(heuresDeVol_)/100*0.1;
    currentValuation_ = std::max(0.0, (1.0 - decote) * purchaseAmount_);  
    //std::cout << "heures de vol : " << static_cast<double>(heuresDeVol_)/100 << std::endl; 
    //std::cout << "decote : " << 1.0-decote << std::endl;             
}

std::ostream& Avion::affiche(std::ostream& out) const{
    std::cout << "---- Avion a " ;
    if (type_ == "REACTION") std::cout << "reaction ---"<< std::endl;
    if (type_ == "HELICES")  std::cout << "helices ---"<< std::endl;
    Vehicule::affiche(out);
    std::cout << "prix actuel : " << currentValuation_ << std::endl;
    out << heuresDeVol_ << " heures de vol." << std::endl;
    return out;
}


